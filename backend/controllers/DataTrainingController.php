...
use backend\models\DataTrainingSearch;
...
class DataTrainingController extends Controller
{
    ...
    public function actionCreatePsikotesBaru()
    {
        $request = Yii::$app->request;
        $idResponden = $request->get('id');
        $model  = DataTraining::find()->where(['id_responden' => $idResponden])->andWhere(['id_attribute' => 5])->one();

        $idAttribute = Attributes::find()->where(['attribute_name' => 'Psikotes'])->one();

        $parameter = Parameter::find()->select(['id', 'value', 'parameter_name'])->where(['id_attribute' => 5])->asArray()->all();
        $newParameter = [];
        ...
        return $this->render('create-pendidikan', [
            ...
            'attribute' => $idAttribute->attribute_name,
            'parameter' => $newParameter,
            'prompt'    => "Pilih Nilai Psikotes"

        ]);
    }


    public function actionCreateIqBaru()
    {
        $request = Yii::$app->request;
        $idResponden = $request->get('id');
        $model  = DataTraining::find()->where(['id_responden' => $idResponden])->andWhere(['id_attribute' => 6])->one();

        $idAttribute = Attributes::find()->where(['attribute_name' => 'IQ'])->one();

        $parameter = Parameter::find()->select(['id', 'value', 'parameter_name'])->where(['id_attribute' => 6])->asArray()->all();
        $newParameter = [];
